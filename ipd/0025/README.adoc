...
inspected first and, if a match is found, the user is authenticated before
`pfexecd` tells the kernel whether the command may proceed. Authentication
happens synchronously, within the upcall, and in a process tree which is
unrelated to the process being authenticated:

- `pfexecd` replies to the initial upcall indicating that authentication
  is required.
- The kernel repeats the upcall, this time attaching a door descriptor for
  the calling process's controlling terminal, opened in-kernel in the same
  way as an open of `/dev/tty` by the caller. If the caller has no
  controlling terminal then the authenticated profile set is not consulted
  at all, and only the unauthenticated profiles apply.
- `pfexecd` runs a helper process - `pfauth` - handing it the terminal.
  The helper grafts the terminal onto its own session using a new terminal
  ioctl, `TIOCGRAFT`, so that `/dev/tty` resolves to it; this is required
  by getpassphrase(3C), used in the PAM conversation, and by any PAM
  module which opens the terminal directly. The graft is one-way - the
  terminal's own session and foreground process group bindings, and hence
  job control in the user's shell, are unaffected. The helper then
  authenticates the user via https://illumos.org/man/3PAM/pam[pam(3PAM)]
  and generates the audit record.
- If authentication succeeds, the reply from `pfexecd` carries the attribute
  overrides from the user's full profile set, and instructs the kernel to
  set the `PRIV_PFEXEC_AUTH` flag on the new process credentials, so that
  commands executed from an authenticated process (for example a profile
  shell) do not prompt again, and so that the flag is visible to
  `chkauthattr_ucred()`. If authentication fails, the reply denies
  execution and the exec(2) fails with `EACCES`.

Because the process being authenticated remains blocked in exec(2)
throughout, and never itself executes any interim helper, its environment
is never exposed to a privileged component and is passed unchanged to the
...
1. When replying after a successful authentication, pfexecd includes the
   authenticated profiles when checking for additional Authorisations and
   exec attributes to assign, and the kernel records the authentication
