...
# There's a slight mix of terminology here, mixing "bind" and "talk to" #
# I tweaked the first one but perhaps use "represent" or similar throughout? #

* `VMADDR_CID_ANY` (`UINT32_MAX`): this CID represents any CID.
  Generally applications are encouraged to bind(3SOCKET) to this to avoid
  needing to know about the current CID.
...
# Since it's unsigned, should we say `UINT_MAX` for the port instead of -1? #
...
# Other OSes have used that value for something else, or should this say #
# NOT used that value for AF_VSOCK? #
...
# Should we consider the implications of ensuring that `SOL_VSOCK` is #
# or is not 40 (I assume it would not be 40)? There will surely be #
# applications with hard-coded values and a note here might be appropriate #
# in the same way we've talked about `AV_VSOCK` #
...
# I assume we are happy that we don't have individual control over `SO_SNDBUF` #
# and `SO_RCVBUF` here? #
...
# Probably doesn't need the ", albeit..." bit given the sentence already #
# has "while it would be quite strange" #
...
# Technically this is a different semantic (socket type vs. protocol name) #
# and I found myself thinking through the `-f` and `-P` interaction a bit #
# more. Probably nothing actionable here, just thinking out loud. #
